...
ini salah: .menuall
ini benar: .menu all
jangan di deketin kalo menu all tapi di spasi ingat
┏━━〔 *LIST MENU* 〕
├❏ ${_p + command} all
├❏ ${_p + command} game
├❏ ${_p + command} rpg 
...
`.trim()
    ...
    await conn.send3ButtonLoc(m.chat, await(await fetch(fla + teks)).buffer(), text.trim(), `Runtime : ${uptime}\n${week} ${date}\nⓁ: limit\nⓟ: premium`, 'Pemilik Bot', `${_p}owner`, 'Donasi', `${_p}donasi`, 'join group Ohlx', `${_p}gcbot`, m)
  } catch (e) {
